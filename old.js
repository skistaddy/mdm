(async () => {
    ...
    function genX(docs, len) {
        return tf.tensor2d(docs.map(d => tokenize(d, len)), [docs.length, len])
    }

    function genY(docs, len) {
        return tf.tensor2d(docs.map(d => {
            x = nlp(format(d.text).join(" "))
                .json()[0]
                .terms
    ...
    vocabulize(data)
    xTensor = genX(data.map(d => d.text), ML)
    ...
    /*console.log(
        xTensor.arraySync().map((x, i1) => {
            return x.map((t, i2) => {
                console.log(t)
                return [kbv(vocab, t), yTensor.arraySync()[i1][i2]]
            })
        })
    )*/
    ...
    function predict(doc){
        fmDoc = format(doc)
        result = model.predict(genX([doc], ML)).arraySync()[0]
